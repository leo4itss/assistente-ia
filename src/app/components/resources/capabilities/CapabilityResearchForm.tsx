...
import type { CapabilityResearch, Source, ValidationErrors } from "@/app/types/assistantConfig";

interface Props {
  capability: CapabilityResearch;
  onChange: (updated: CapabilityResearch) => void;
  sources: Source[];
  errors: ValidationErrors;
  errorPrefix: string;
}

export default function CapabilityResearchForm({ capability, onChange, sources, errors, errorPrefix }: Props) {
  const e = (f: string) => errors[`${errorPrefix}.${f}`];
  const update = (patch: Partial<CapabilityResearch>) => onChange({ ...capability, ...patch });

  return (
    <div className="flex flex-col gap-[16px]">
      <div className="flex flex-col gap-[8px]">
        <FieldLabel required>ID</FieldLabel>
        <TextInput value={capability.name} onChange={(v) => update({ name: v })} placeholder="web" />
      ...
      <SourceBindSelect
        ...
        sources={sources}
        compatibleKinds={CAPABILITY_SOURCE_COMPATIBILITY.research}
        errors={errors}
      ...
      <div className="flex flex-col gap-[8px]">
        ...
        <TextareaInput
          ...
          placeholder="Como o agente deve conduzir pesquisas com esta ferramenta."
          rows={3}
        />
