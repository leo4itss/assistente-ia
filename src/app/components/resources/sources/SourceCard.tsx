...
interface SourceCardProps {
  source: Source;
  /** Quantas capabilities usam esta source (badge de relação — etapa 2 popula). */
  usedByCount?: number;
  expanded: boolean;
  onToggle: () => void;
  onDelete: () => void;
  ...
  return (
    <div className="bg-[#111827] relative rounded-[14px] w-full">
      ...
      <div className="flex items-center gap-[12px] px-[20px] py-[14px]">
        ...
        >
          <svg
            className={`size-[14px] transition-transform ${expanded ? "rotate-90" : ""}`}
            fill="none"
            viewBox="0 0 14 14"
          >
        ...
        <button onClick={onToggle} className="flex items-center gap-[8px] min-w-0 flex-1 text-left">
          {(source.kind === "documents" || source.kind === "mcp") && source.external_id ? (
            <>
        ...
        {usedByCount > 0 && (
          <span
            ...
            title="Capacidades que usam esta fonte"
          >
            {usedByCount} {usedByCount === 1 ? "capacidade" : "capacidades"}
          </span>
        )}
