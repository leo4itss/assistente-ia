...
export default function DeleteConfirmationDialog({ isOpen, onClose, onConfirm }: DeleteConfirmationDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      {/* Dialog Content */}
      <div className="bg-[#111827] flex flex-col gap-[32px] items-end p-[24px] relative rounded-[10px] w-full max-w-[425px] shadow-xl animate-in fade-in zoom-in-95 duration-200 z-10" data-name="Dialog">
        ...
        <div className="content-stretch flex gap-[8px] items-center justify-end relative shrink-0 w-full" data-name="Dialog Footer">
          ...
          <button 
            onClick={onClose}
            className="bg-[rgba(255,255,255,0.05)] content-stretch flex gap-[8px] h-[36px] items-center justify-center px-[16px] py-[8px] relative rounded-[8px] shrink-0 hover:bg-[rgba(255,255,255,0.1)] transition-colors"
          >
        ...
        <button 
          onClick={onClose}
          className="absolute right-[16px] rounded-[2px] size-[16px] top-[16px] hover:opacity-80 transition-opacity"
        >
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
            <g id="Icon / X" opacity="0.7">
              <path d="M12 4L4 12M4 4L12 12" id="Vector" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33" className="text-[#F9FAFB]" />
            </g>
          </svg>
        </button>
      </div>
